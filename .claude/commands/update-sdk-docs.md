...
You are documenting the `@flowcore/sdk` package. Given a domain name (e.g., `data-pathways`, `tenants`, `iam`), you will:
...
For each command:
- Name — ClassName (auth, METHOD `/path`)
\`\`\`ts
export interface InputType {
  // all fields with types
}
// Output: OutputType { fields }
await client.execute(new ClassName({ ... }))
\`\`\`
